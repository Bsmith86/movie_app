...
const Search = (props) => {
    ...
    useEffect(() => {
        const moviesArray = ["Till", "Plane", "House Party", "National Treasure", "M3gan", "I Spit on your grave", "Demolition Man", "Titanic", "Bad Boys", "Rush Hour"];
        // make this movie call randomly choose between 10 movies
        if (isFirstRender.current === true) {
            console.log("making api call");
            isFirstRender.current = false;
            makeServerCall(moviesArray[Math.floor(Math.random() * 11)])
        }
    }, []);
